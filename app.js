    ...
    // ==========================================
    // 🎨 ตัวแปรสีหลัก (COLORS Object) — อัปเดตตามพาเลทใหม่
    // ==========================================
    const COLORS = {
      blue: '#b3c3d2',       // สีพื้น section บน
      pink: '#cdcea1',       // สี blob พื้นหลัง (top-left)
      purple: '#eaecea',     // สี blob พื้นหลัง (กลาง)
      orange: '#b3c3d2',     // สีพื้น NEXT QUEUE section
      bg: '#eaecea',         // ⭐ สีพื้นหลังทั้งเพจ
      white: '#ffffff',      // สีใช้แทน white ใน border บางจุด
      textMain: '#6f86aa',   // ⭐ สีหลักของข้อความและปุ่มหลัก
      textSoft: '#889688',   // สีข้อความรอง
      pinkDeep: '#6f86aa',   // สี accent เข้ม (ปุ่ม, badge)
      blueDeep: '#6f86aa',   // สี accent เข้ม (title, icon)
      purpleDeep: '#889688', // สี accent เข้ม (label, text)
      orangeDeep: '#889688', // สี accent เข้ม (NEXT QUEUE label)
      cardBg: '#ffffff',     // ⭐ สีพื้นหลังการ์ดงาน (todo/doing)
      cardBgDone: '#cdcea1', // ⭐ สีพื้นหลังการ์ดงาน (done)
    ...
    const Header = React.memo(({ isAdmin, setIsAdmin, setShowAuthModal, handleImageUpload, profileImage }) => (
      <div className="px-5 pt-6 pb-2 relative z-10">
        <div className="backdrop-blur-md rounded-[32px] p-4 shadow-xl shadow-[#6f86aa]/20 flex items-center justify-between h-20 relative overflow-hidden group" style={{ backgroundColor: `${COLORS.cardWhite}66`, borderWidth: '1px', borderColor: `${COLORS.cardBorder}80` }}>
          <div className="absolute left-6 top-1/2 -translate-y-1/2 w-24 h-8 bg-[#b3c3d2]/40 blur-xl rounded-full"></div>
          <div className="flex flex-col justify-center ml-2 relative z-10">
            <div className="flex items-center gap-1">
             <h1 className="text-2xl font-black italic tracking-tighter flex">
              {"LemonTart".split('').map((char, index) => (
                <span 
                  key={index} 
                  className={`bg-gradient-to-br from-[#6f86aa] to-[#b3c3d2] bg-clip-text text-transparent drop-shadow-sm inline-block animate-text-wave pb-2 ${char === ' ' ? 'w-2.5' : 'pr-[1.5px]'}`} 
    ...
    const ProgressFlower = React.memo(({ progress, todayTasksLength, doneToday, triggerFlowerBounce, isBouncing }) => {
      ...
      return (
        <div onClick={triggerFlowerBounce} className={`backdrop-blur-sm rounded-[28px] p-3 flex flex-col items-center justify-center shadow-lg shadow-[#6f86aa]/20 h-full relative overflow-hidden transition-all duration-300 cursor-pointer ${isBouncing ? 'scale-95' : 'hover:scale-[1.02]'}`} style={{ backgroundColor: `${COLORS.cardWhite}cc`, borderWidth: '1px', borderColor: COLORS.cardBorder }}>
          ...
          <div className="relative w-full h-20 flex items-center justify-center mt-2">
            {progress > 0 && (
              <div className="absolute inset-0 pointer-events-none">
                  <div className="absolute top-1/2 left-1/4 w-1.5 h-1.5 bg-[#6f86aa] rounded-full animate-float-particle-1 opacity-60"></div>
                  <div className="absolute top-2/3 right-1/4 w-2 h-2 bg-[#889688] rounded-full animate-float-particle-2 opacity-60"></div>
                  <div className="absolute top-1/3 right-1/3 w-1.5 h-1.5 bg-[#b3c3d2] rounded-full animate-float-particle-3 opacity-60"></div>
              </div>
            )}
              ...
              <g transform="translate(50, 50)">
                  <g className="animate-breathe">
                    <g transform={`scale(${bloomScale})`} className="transition-transform duration-1000 cubic-bezier(0.34, 1.56, 0.64, 1)">
                      <g transform="translate(-50, -50)">
                        {petals.map((p, i) => (
                          <circle key={i} cx={p.cx} cy={p.cy} r={p.r} fill="#b3c3d2" opacity="0.95" className="transition-all duration-700"/>
                        ))}
                        <circle cx="50" cy="50" r="14" fill="#cdcea1" className="transition-all duration-700" />
                        {isFullBloom && (
                          <g transform="translate(50, 50)" className="animate-fade-in">
                            ...
                            <ellipse cx="-6" cy="1" rx="1.5" ry="1" fill="#6f86aa" opacity="0.6"/>
                            <ellipse cx="6" cy="1" rx="1.5" ry="1" fill="#6f86aa" opacity="0.6"/>
                          </g>
                        )}
                      </g>
          ...
          <div className="mt-1 flex items-center gap-1 px-2 py-0.5 rounded-full z-10" style={{ backgroundColor: `${COLORS.buttonBg}99`, borderWidth: '1px', borderColor: COLORS.cardBorder }}>
            ...
            <div className="w-1 h-1 rounded-full bg-[#b3c3d2]"></div>
    ...
    const CompactStats = React.memo(({ isAdmin, selectedNextDate, nextQueueDate, handleSetManualDate, waitingCount }) => (
      <div className="backdrop-blur-sm rounded-[28px] p-1 shadow-lg shadow-[#6f86aa]/20 h-full flex overflow-hidden relative group transition-all" style={{ backgroundColor: `${COLORS.cardWhite}cc`, borderWidth: '1px', borderColor: COLORS.cardBorder }}>
          <div className={`flex-1 flex flex-col justify-center items-center p-2 relative rounded-l-[24px] transition-colors overflow-hidden ${isAdmin ? 'hover:bg-white/50 cursor-pointer' : ''}`} style={{ backgroundColor: `${COLORS.orange}66` }}>
            ...
            <div className="absolute top-2 left-1/2 -translate-x-1/2 px-2 py-0.5 rounded-md text-[7px] font-black shadow-sm border border-[#b3c3d2] uppercase tracking-wider whitespace-nowrap" style={{ color: COLORS.orangeDeep, backgroundColor: COLORS.buttonBg }}>NEXT QUEUE</div>
            ...
            {isAdmin && selectedNextDate && (
              <button 
                ...
                className="absolute bottom-1.5 left-1/2 -translate-x-1/2 z-20 px-2.5 py-0.5 rounded-md text-[7px] font-black shadow-sm border border-[#b3c3d2] hover:bg-[#eaecea] active:scale-90 transition-all"
          ...
          <div className="flex-1 flex flex-col justify-center items-center p-2 relative">
            <div className="absolute top-2 left-1/2 -translate-x-1/2 px-2.5 py-0.5 rounded-md text-[7px] font-black shadow-sm border border-[#b3c3d2] uppercase tracking-wider whitespace-nowrap z-10" style={{ color: COLORS.blueDeep, backgroundColor: COLORS.buttonBg }}>จำนวนคิวก่อนหน้า</div>
            ...
            <div className="relative mt-5 flex flex-col items-center">
               <div className="relative">
                 ...
                 <div className="w-11 h-11 rounded-full flex flex-col items-center justify-center shadow-[inset_0_2px_8px_rgba(0,0,0,0.02)] relative z-10" style={{ backgroundColor: COLORS.cardInner, borderWidth: '2px', borderColor: COLORS.blue }}>
                    <span className="text-xl font-black leading-none" style={{ color: COLORS.blueDeep }}>{waitingCount}</span>
                 </div>
               </div>
    ...
    // ==========================================
    const TaskCard = React.memo(({ task, isAdmin, onEdit, onToggleDoing, onToggleStatus }) => (
      <div onClick={() => isAdmin && onEdit(task)} className={`relative mb-2 rounded-[22px] p-3 transition-all shadow-sm group backdrop-blur-sm ${isAdmin ? 'active:scale-[0.97] cursor-pointer' : ''}`} 
        style={{ 
          backgroundColor: task.status === 'done' ? COLORS.cardBgDone : task.status === 'doing' ? COLORS.cardBg : COLORS.cardBg,
          borderWidth: '1px',
          borderStyle: 'solid',
          borderColor: task.status === 'doing' ? COLORS.orangeDeep : (task.status === 'done' ? 'transparent' : COLORS.white), 
          opacity: task.status === 'done' ? 0.7 : 1 
        }}>
         {task.status === 'doing' && (
           <div className="absolute inset-0 pointer-events-none z-0">
             {/* เปลี่ยนจากอีโมจิ เป็นแท็ก img และใส่ลิงก์รูป .webp ของคุณใน src */}
             {/* สามารถปรับขนาดรูปได้ที่ w-4 h-4 (ยิ่งตัวเลขเยอะ รูปยิ่งใหญ่ เช่น w-6 h-6) */}
             
             <img src="https://i.ibb.co/LzjNBZnZ/10.webp" alt="particle" className="absolute -bottom-2 -left-3 w-5 h-5 object-contain animate-float-particle-1 drop-shadow-sm" />
             
             <img src="https://i.ibb.co/LzjNBZnZ/10.webp" alt="particle" className="absolute top-[30%] -left-4 w-4 h-4 object-contain animate-float-particle-3 drop-shadow-sm" style={{ animationDelay: '1.5s' }} />
             
             <img src="https://i.ibb.co/LzjNBZnZ/10.webp" alt="particle" className="absolute -bottom-2 -right-3 w-5 h-5 object-contain animate-float-particle-2 drop-shadow-sm" style={{ animationDelay: '0.5s' }} />
             
             <img src="https://i.ibb.co/LzjNBZnZ/10.webp" alt="particle" className="absolute top-[30%] -right-4 w-4 h-4 object-contain animate-float-particle-1 drop-shadow-sm" style={{ animationDelay: '1.2s' }} />
           </div>
        )}
        <div className="absolute left-2 top-3 bottom-3 w-1 rounded-full shadow-sm z-10" style={{ backgroundColor: task.channel === 'LINE' ? '#cdcea1' : task.channel === 'FB' || task.channel === 'FACEBOOK' ? '#b3c3d2' : task.channel === 'IG' ? '#eaecea' : COLORS.purpleDeep }}></div>
        <div className="pl-3 pr-1 relative z-10">
          <div className="flex justify-between items-start mb-1">
            <div className="flex items-center gap-2 min-w-0">
              <span className="font-bold text-base truncate" style={{ color: COLORS.textMain }}>{task.client}</span>
              <span className={`text-[7px] px-1.5 py-0.5 rounded-md font-black uppercase border ${
                task.channel === 'LINE' ? 'text-[#889688] bg-[#cdcea1]/20 border-[#889688]/20' : 
                task.channel === 'FB' || task.channel === 'FACEBOOK' ? 'text-[#6f86aa] bg-[#b3c3d2]/20 border-[#6f86aa]/20' : 
                task.channel === 'IG' ? 'text-[#889688] bg-[#eaecea]/50 border-[#cdcea1]/30' : 
                task.channel === 'DISCORD' || task.channel === 'Discord' ? 'text-[#6f86aa] bg-[#b3c3d2]/10 border-[#6f86aa]/20' : 
                'bg-white/50 text-gray-400 border-gray-100'
              }`}>{task.channel}</span>
              {task.priority === 'urgent' && <span className="text-[9px] font-bold animate-pulse px-2 py-0.5 rounded-full shadow-sm" style={{ color: COLORS.pinkDeep, backgroundColor: COLORS.buttonBg }}>{'\u{1F525}'} ด่วน</span>}
            </div>
            <div className="flex items-center gap-1 text-[9px] font-black bg-[#eaecea]/80 px-2.5 py-0.5 rounded-lg border border-[#b3c3d2] shadow-sm" style={{ color: COLORS.pinkDeep }}>
                <IconCalendar size={11} style={{ color: COLORS.pinkDeep }} />
          ...
          <div className="flex justify-between items-end mt-1">
            <div className="flex-1 min-w-0 mr-4 flex flex-col">
              <span className={`text-[10px] font-bold truncate ${task.status === 'done' ? 'line-through text-gray-400' : 'text-gray-500'}`}>{task.type}</span>
              {task.details && (
                <span className={`text-[11px] font-medium truncate mt-0.5 ${task.status === 'done' ? 'text-gray-400' : 'text-gray-600'}`}>{task.details}</span>
              )}
            </div>

            {isAdmin ? (
              <div className="flex gap-1.5">
                <button onClick={(e) => onToggleDoing(e, task.id)} className={`relative w-[28px] h-[28px] rounded-[8px] flex items-center justify-center transition-all active:scale-90 shadow-[0_3px_10px_rgba(179,195,210,0.5)]`} style={{ color: task.status === 'doing' ? COLORS.orangeDeep : '#B0BEC5', backgroundColor: task.status === 'doing' ? '#eaecea' : COLORS.buttonBg, borderWidth: '2px', borderColor: `${COLORS.cardBorder}cc` }}>
                  <div className={`${task.status === 'doing' ? 'animate-spin-slow' : ''} flex items-center justify-center`}>
                    <IconLoader size={14} />
                  </div>
                </button>
                <button onClick={(e) => onToggleStatus(e, task.id)} className={`relative w-[28px] h-[28px] rounded-[8px] flex items-center justify-center transition-all active:scale-90 shadow-[0_3px_10px_rgba(179,195,210,0.5)]`} style={{ color: task.status === 'done' ? COLORS.pinkDeep : '#B0BEC5', backgroundColor: task.status === 'done' ? '#eaecea' : COLORS.buttonBg, borderWidth: '2px', borderColor: `${COLORS.cardBorder}cc` }}>
                  <IconCheck size={16} />
                </button>
              </div>
            ) : (
              <div className="flex items-center">
                {task.status === 'doing' && <span className="text-[10px] font-bold text-[#889688] flex items-center gap-1 bg-[#eaecea] px-2 py-1 rounded-md border border-[#b3c3d2] shadow-sm"><IconLoader size={10} className="animate-spin-slow"/> กำลังทำ</span>}
                {task.status === 'done' && <span className="text-[10px] font-bold text-[#6f86aa] flex items-center gap-1 px-2 py-1 rounded-md border border-[#b3c3d2] shadow-sm" style={{ backgroundColor: COLORS.buttonBg }}><IconCheck size={10} /> เสร็จแล้ว</span>}
                {task.status === 'todo' && <span className="text-[10px] font-bold text-gray-400 flex items-center gap-1 px-2 py-1 rounded-md border border-gray-100 shadow-sm" style={{ backgroundColor: COLORS.buttonBg }}><IconClock size={10} /> รอคิว</span>}
              </div>
            )}
          </div>
        </div>
    ...
    const CalendarView = React.memo(({ currentMonth, setCurrentMonth, selectedDate, setSelectedDate, today, tasks }) => {
      ...
      return (
        <div className="backdrop-blur-xl rounded-[32px] p-5 shadow-[0_12px_30px_rgba(179,195,210,0.4)] mb-5 animate-fadeIn" style={{ backgroundColor: `${COLORS.calendarBg}e6`, borderWidth: '4px', borderColor: COLORS.cardBorder }}>
          <div className="flex justify-between items-center mb-4 px-2">
            <button onClick={() => setCurrentMonth(new Date(year, month - 1, 1))} className="p-1.5 hover:bg-[#b3c3d2] rounded-full transition-colors text-[#889688]"><IconChevronLeft size={20}/></button>
            <h3 className="font-black text-[16px] text-[#6f86aa] tracking-widest">{monthNames[month]} {year}</h3>
            <button onClick={() => setCurrentMonth(new Date(year, month + 1, 1))} className="p-1.5 hover:bg-[#b3c3d2] rounded-full transition-colors text-[#889688]"><IconChevronRight size={20}/></button>
          </div>
          
          <div className="grid grid-cols-7 gap-1 text-center mb-3">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
              <div key={d} className="text-[9px] font-black text-[#889688] opacity-60 uppercase">{d}</div>
            ))}
          </div>
                ...
                <div key={dateStr} className="flex justify-center items-center h-10">
                  <div 
                    ...
                    className={`relative w-10 h-10 flex flex-col items-center justify-center cursor-pointer transition-all ${
                      isSelected 
                        ? 'bg-[#6f86aa] text-white rounded-full font-black shadow-md scale-105'
                        : isToday 
                          ? 'text-[#6f86aa] font-black bg-[#b3c3d2] rounded-full'
                          : 'text-[#889688] font-bold hover:bg-[#eaecea] hover:shadow-sm rounded-full'
                    }`}
    ...
    function App() {
      ...
      const WEB_APP_URL = "https://script.google.com/macros/s/AKfycbz8BkEQftLrwH43S0aY-mT6JzCU3J0t_0gRjGAJ77Ay_WJ45kpOO7P7JCrfK5lBrc8J/exec";
      ...
      const handleAdminLogin = (e) => {
        ...
        fetchAPI({ action: 'verifyAdminPassword', inputPassword: password })
          .then(res => {
      ...
      return (
        <div className="min-h-screen font-sans relative" style={{ backgroundColor: COLORS.bg, color: COLORS.textMain }}>
          ...
          <div className="max-w-md mx-auto min-h-screen relative pb-28">
            <Header isAdmin={isAdmin} setIsAdmin={setIsAdmin} setShowAuthModal={setShowAuthModal} handleImageUpload={handleImageUpload} profileImage={profileImage} />
            
            <div className="px-5 relative z-10 mb-4">
               {bannerImage && isBannerVisible && (
                 <div className="relative group w-full h-32 rounded-[24px] overflow-hidden shadow-sm backdrop-blur-sm" style={{ borderWidth: '3px', borderColor: COLORS.cardBorder, backgroundColor: `${COLORS.cardWhite}66` }}>
                    <img src={bannerImage} alt="Shop Banner" className="w-full h-full object-cover" />
                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                       {isAdmin && (
                         <button onClick={() => setShowBannerModal(true)} className="p-1.5 backdrop-blur-md rounded-full shadow-sm text-gray-500 hover:text-[#6f86aa] transition-colors" style={{ backgroundColor: `${COLORS.bannerOverlayBg}e6` }} title="เปลี่ยนแบนเนอร์">
                            <IconImage size={16} />
                         </button>
                       )}
                       {isAdmin && (
                         <button onClick={() => toggleBannerVisibility(false)} className="p-1.5 backdrop-blur-md rounded-full shadow-sm text-gray-500 hover:text-[#889688] transition-colors" style={{ backgroundColor: `${COLORS.bannerOverlayBg}e6` }} title="ซ่อนแบนเนอร์">
                            <IconEyeOff size={16} />
                         </button>
                       )}
                    </div>
               ...
               {!bannerImage && isAdmin && (
                 <div onClick={() => setShowBannerModal(true)} className="w-full h-16 rounded-[24px] border-[3px] border-dashed backdrop-blur-sm flex items-center justify-center cursor-pointer hover:opacity-80 transition-colors" style={{ borderColor: COLORS.cardBorder, backgroundColor: `${COLORS.cardWhite}66` }}>
              ...
              {view === 'dashboard' ? (
                <div className="animate-fadeIn">
                  <div className="flex gap-3 h-28 mb-4">
                    ...
                    <div className="flex-1">
                      <ProgressFlower progress={progress} todayTasksLength={todayTasks.length} doneToday={doneToday} triggerFlowerBounce={triggerFlowerBounce} isBouncing={isBouncing} />
                    </div>
                  </div>
                  <div className="flex items-center justify-between mb-2.5 ml-1">
                    ...
                    <div className="h-[1px] flex-1 bg-gradient-to-r from-[#b3c3d2]/40 to-transparent mx-3"></div>
                  </div>
                  <div className="space-y-1">
                    {todayTasks.length > 0 ? todayTasks.map(t => <TaskCard key={t.id} task={t} isAdmin={isAdmin} onEdit={handleEditTask} onToggleDoing={toggleDoing} onToggleStatus={toggleStatus} />) : (
                      <div className="text-center py-10 rounded-[28px] border-2 border-dashed" style={{ backgroundColor: `${COLORS.cardWhite}4d`, borderColor: '#b3c3d280' }}>
                ...
                <div className="animate-fadeIn">
                  <div className="relative mb-4">
                    ...
                    <input type="text" placeholder="ค้นหาคิวงาน..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full rounded-[22px] py-3 pl-10 pr-4 outline-none text-xs shadow-lg shadow-[#6f86aa]/10 transition-all backdrop-blur-sm" style={{ backgroundColor: `${COLORS.searchInputBg}cc`, borderWidth: '2px', borderColor: COLORS.blue }} />
                  </div>
                  
                  <div className="flex gap-2 mb-3 p-1.5 rounded-full backdrop-blur-md shadow-sm" style={{ backgroundColor: `${COLORS.filterBarBg}80`, borderWidth: '3px', borderColor: COLORS.cardBorder }}>
                      <button onClick={() => setFilterType('all')} className={`flex-1 text-[9px] font-black py-2 rounded-full transition-all tracking-wider ${filterType === 'all' ? 'shadow-sm' : 'text-gray-400'}`} style={{ color: filterType === 'all' ? COLORS.pinkDeep : '', backgroundColor: filterType === 'all' ? COLORS.buttonBg : 'transparent' }}>ทั้งหมด</button>
                      <button onClick={() => setFilterType('pending')} className={`flex-1 text-[9px] font-black py-2 rounded-full transition-all tracking-wider ${filterType === 'pending' ? 'shadow-sm' : 'text-gray-400'}`} style={{ color: filterType === 'pending' ? COLORS.orangeDeep : '', backgroundColor: filterType === 'pending' ? COLORS.buttonBg : 'transparent' }}>ยังไม่ได้เริ่ม</button>
                      <button onClick={() => setFilterType('done')} className={`flex-1 text-[9px] font-black py-2 rounded-full transition-all tracking-wider ${filterType === 'done' ? 'shadow-sm' : 'text-gray-400'}`} style={{ color: filterType === 'done' ? COLORS.blueDeep : '', backgroundColor: filterType === 'done' ? COLORS.buttonBg : 'transparent' }}>เสร็จแล้ว</button>
                  </div>
                  ...
                  <div className="flex justify-end mb-4 pr-1">
                    ...
                    }} className="flex items-center gap-1.5 px-3 py-1.5 backdrop-blur-md rounded-full shadow-[0_4px_10px_rgba(179,195,210,0.5)] text-[9px] font-black uppercase tracking-widest hover:text-[#6f86aa] transition-all active:scale-95" style={{ color: COLORS.textSoft, backgroundColor: `${COLORS.buttonBg}cc`, borderWidth: '2px', borderColor: COLORS.cardBorder }}>
                  ...
                  <div className="space-y-1">
                    {displayedTasks.length > 0 ? displayedTasks.map(t => <TaskCard key={t.id} task={t} isAdmin={isAdmin} onEdit={handleEditTask} onToggleDoing={toggleDoing} onToggleStatus={toggleStatus} />) : (
            ...
            <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-[210px] z-50">
              <div className="backdrop-blur-xl rounded-full h-14 shadow-2xl shadow-[#6f86aa]/30 flex items-center justify-between px-6 relative" style={{ backgroundColor: `${COLORS.navBarBg}99`, borderWidth: '1px', borderColor: COLORS.cardBorder }}>
                  ...
                  {isAdmin && (
                    <div className="absolute left-1/2 -translate-x-1/2 -top-6">
                      <button onClick={() => { setEditingTask(null); setShowModal(true); }} className="w-14 h-14 rounded-full text-white flex items-center justify-center shadow-xl shadow-[#6f86aa]/50 active:scale-90 transition-transform" style={{ background: `linear-gradient(to bottom right, ${COLORS.textMain}, ${COLORS.textSoft})`, borderWidth: '4px', borderColor: COLORS.cardBorder }}>
                        <IconPlus size={24} />
                      </button>
                    </div>
                  )}
                   ...
                   <div className="space-y-2 relative">
                     ...
                     <div className="flex justify-end mb-1">
                       <label className="flex items-center cursor-pointer gap-2 px-3 py-1.5 rounded-full shadow-sm border border-gray-100" style={{ backgroundColor: COLORS.buttonBg }}>
                         <span className="text-[10px] font-bold text-gray-500">คิวด่วน 🔥</span>
                         <div className="relative">
                     ...
                     <input name="client" required defaultValue={editingTask?.client || ''} className="w-full p-3.5 rounded-2xl border border-gray-100 outline-none text-[11px] shadow-sm focus:border-[#eaecea] transition-all" style={{ backgroundColor: COLORS.cardInner }} placeholder="ชื่อลูกค้า" />
                     ...
                     <div className="flex flex-col gap-2">
                        ...
                        <input list="channelsList" name="channel" required defaultValue={editingTask?.channel || ''} className="w-full p-3.5 rounded-2xl font-black text-[10px] outline-none border border-gray-100 shadow-sm" style={{ backgroundColor: COLORS.cardInner }} placeholder="ช่องทาง" />
                        <datalist id="channelsList">
                          {channels.map(ch => <option key={ch} value={ch} />)}
                        </datalist>
                     </div>

                     <textarea name="details" rows="2" defaultValue={editingTask?.details || ''} className="w-full p-3.5 rounded-2xl border border-gray-100 outline-none text-[11px] shadow-sm focus:border-[#eaecea] transition-all resize-none" style={{ backgroundColor: COLORS.cardInner }} placeholder="รายละเอียดเพิ่มเติม (ถ้ามี)..."></textarea>
                     
                     <div className="flex items-center justify-between bg-[#eaecea]/30 p-1.5 pl-4 rounded-2xl border border-[#b3c3d2]/50 transition-all hover:bg-[#eaecea]/80 hover:shadow-sm group mt-1">
                       <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-black" style={{ color: COLORS.pinkDeep }}>
                         <IconCalendar size={12} className="group-hover:animate-bounce-slow transition-transform" /><span>Deadline</span>
                       </div>
                       <input name="deadline" type="date" required defaultValue={editingTask?.deadline || nextQueueDate} className="w-32 p-2.5 rounded-xl border border-[#b3c3d2] outline-none text-[10px] font-black text-[#6f86aa] shadow-sm focus:border-[#6f86aa] focus:ring-2 focus:ring-[#6f86aa]/50 transition-all cursor-pointer active:scale-95 hover:border-[#6f86aa]" style={{ backgroundColor: COLORS.cardInner }} />
                     </div>
              ...
              <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/10 backdrop-blur-[2px] animate-fadeIn">
                <form onSubmit={handleAdminLogin} className="backdrop-blur-xl w-full max-w-xs rounded-[35px] p-6 shadow-2xl animate-slideUp" style={{ backgroundColor: `${COLORS.modalBg}f2`, borderWidth: '1px', borderColor: COLORS.cardBorder }}>
                   ...
                   <div className="space-y-2">
                     <input type="password" required value={password} onChange={(e) => { setPassword(e.target.value); setAuthError(false); }} className={`w-full p-3.5 rounded-2xl border outline-none text-[11px] shadow-sm transition-all text-center tracking-widest ${authError ? 'border-red-300 focus:border-red-400' : 'border-gray-100 focus:border-[#eaecea]'}`} style={{ backgroundColor: COLORS.cardInner }} placeholder="ใส่รหัสผ่าน" disabled={isVerifyingAuth} />
            ...
            {showImageModal && (
              <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-black/10 backdrop-blur-[2px] animate-fadeIn">
                <form onSubmit={(e) => { 
                   ...
                   <div className="space-y-2">
                     <input type="url" required value={tempImageUrl} onChange={(e) => setTempImageUrl(e.target.value)} className="w-full p-3.5 rounded-2xl border border-gray-100 outline-none text-[11px] shadow-sm focus:border-[#eaecea] transition-all text-center" style={{ backgroundColor: COLORS.cardInner }} placeholder="วางลิงก์รูปภาพ Google Drive ที่นี่..." />
            ...
            {showBannerModal && (
              <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-black/10 backdrop-blur-[2px] animate-fadeIn">
                <form onSubmit={(e) => { 
                   ...
                   <div className="space-y-2">
                     <input type="url" required value={tempBannerUrl} onChange={(e) => setTempBannerUrl(e.target.value)} className="w-full p-3.5 rounded-2xl border border-gray-100 outline-none text-[11px] shadow-sm focus:border-[#eaecea] transition-all text-center" style={{ backgroundColor: COLORS.cardInner }} placeholder="วางลิงก์รูปภาพ Google Drive..." />
